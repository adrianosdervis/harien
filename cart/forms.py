...
from shop.models import ColourVariation, Product, SizeVariation
...
class AddToCartForm(forms.ModelForm):
    colour = forms.ModelChoiceField(
        queryset=ColourVariation.objects.none(), empty_label='Available colours')
    size = forms.ModelChoiceField(
        queryset=SizeVariation.objects.none(), empty_label='Available sizes')

    class Meta:
        model = OrderItem
        fields = ['quantity', 'colour', 'size']

    # initialize the form and pass in the product_id so that we can get the product and set the queryset to be only the available colours for this product
    def __init__(self, *args, **kwargs):
        # product_id will be passed from the ProductDetailView's get_form_kwargs method
...
class BankForm(forms.Form):
    bank_transfer_id = forms.CharField(max_length=200)
